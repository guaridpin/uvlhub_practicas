...
# @decorador (función que modifica o extiende el comportamiento de otra función)


@notepad_bp.route('/notepad', methods=['GET'])
@login_required
def index():
    # Se crea un formulario para añadir notas nuevas
    form = NotepadForm()
    # Se obtiene un listado de todas las notas creadas por el usuario actual
    notepads = notepad_service.get_all_by_user(current_user.id)
    # Se renderiza el resultado en una plantilla
